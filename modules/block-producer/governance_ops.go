...
// single block can carry. Generous enough to clear normal governance traffic
...
// MakeSafetySlashReverses drains up to maxGovernanceOpsPerBlock
// vsc.safety_slash_reverse entries from PendingGovOps, encodes each as
// DAG-CBOR, pins the bytes to the supplied DataLayer session, and
// returns the resulting block-tx stubs. The leader's inclusion is the
// authoring step; the 2/3 BLS aggregate over the carrying block is the
// auth gate; the apply path is the policy gate (it independently
// re-validates against on-chain state and silently drops invalid
// entries).
//
