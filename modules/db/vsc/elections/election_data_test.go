...
func TestElectionDataCid(t *testing.T) {
	// Pinned CID for the canonical-form ElectionData with the consensus version
	// triple (version_major / version_non_consensus) encoded. The optional
	// pendulum `Settlement *SettlementRecord` field must NOT change this CID when
	// nil — refmt must omit the key, not encode it as CBOR null, or the encoding
	// diverges from historical bytes on replay. DAG-CBOR sorts map keys
	// canonically, so struct field order does not affect this value.
