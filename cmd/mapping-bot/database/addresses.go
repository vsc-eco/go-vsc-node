...
func (a *AddressStore) Insert(ctx context.Context, chainAddr, instruction string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to insert address mapping [chainAddr:%s]: %w", chainAddr, err)
	}
	return nil
}

// GetInstruction retrieves the instruction for a given BTC address
func (a *AddressStore) GetInstruction(ctx context.Context, chainAddr string) (string, error) {
	var result AddressMapping
	err := a.collection.FindOne(ctx, bson.M{"_id": chainAddr}).Decode(&result)
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return "", ErrAddrNotFound
		}
		return "", fmt.Errorf("failed to get instruction [chainAddr: %s]: %w", chainAddr, err)
	}
